...
def main():
    ...
    ctl_out = subprocess.run([f"{args.venv_dir}/bin/primazactl","install","main",
    ...
    for i in range(1,40):
        pods = subprocess.run(["kubectl","get","pods","-n","primaza-system"],
                              capture_output=True)
        print(f'{i} {time.strftime("%I:%M:%S %p", time.localtime())}. kubectl get pods -n primaza-system:')
