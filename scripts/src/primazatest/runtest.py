...
def check_dry_run(manifest_file, resp, expected_last_message):
    ...
    if not resp:
        ...
        if num_lines < expect_lines:
            ...
            print(f"[{FAIL}] last line of response was not {expected_last_message}: "
                  f"{resp}")
            outcome = False
...
def check_output(manifest_file, resp):
    ...
    for resource in manifest_list:
        resource_string = yaml.dump(resource).strip()
        for line in resource_string.splitlines():
            if line not in resp:
                outcome = False
                print(f"resource not in response:\n:{line}:\n\n")
